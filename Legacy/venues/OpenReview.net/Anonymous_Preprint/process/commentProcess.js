function(){
    ...
    // origNote is the submitted paper
    // replytoNote is the previous comment if there is one
    ...
    Promise.all([
      ...
      // selfComment true if comment is directly on comment/submission by the same person
      var selfComment = replytoNoteSignatures.indexOf(note.signatures[0]) > -1;

      console.log('Anon Preprint public comment: self comment detected');
      ...
      if(!selfComment){
        if(replytoNote.id == origNote.id){
          ...
          author_mail = {
            ...
            "message": "Your submission to " + CONFERENCEPHRASE + " entitled \""+origNote.content.title+"\" has received a comment.\n\nComment title: " + note.content.title + "\n\nComment: " + note.content.comment + "\n\nTo view the comment, click here: " + baseUrl + "/forum?id=" + note.forum + '&noteId=' + note.id
          };
        } else {
          author_mail = {
            "groups": replytoNote.signatures,
