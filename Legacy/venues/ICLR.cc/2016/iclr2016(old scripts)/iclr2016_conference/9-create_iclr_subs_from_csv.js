...
function submit_paper(token, reader, cb) {
	var record = reader.read()
	if (record) {
		processedrecord = process_record(record)
		var paperSubmission = new or3post(noteUrl, processedrecord, loggedInHdr(token));
		request(paperSubmission, cb)
	};
}

function make_post_req(url, o, reader) {
    ...
    request(loginReq, function(error, response, body) {
      if (!error && response.statusCode == 200) {
        var token = body.token;
		  ...
		  */
		  var submit_paper_callback = function(error, response, body){
			  console.log("holler back");
			  console.log(body);
			  console.log(response);
			  submit_paper(token, reader, submit_paper_callback);
		  }
		  submit_paper(token, reader, submit_paper_callback)
    }
  });
