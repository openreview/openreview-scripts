...
"""
Outputs list of reviewers that haven't reviewed their assigned papers yet.
The list consists of: paper numbers, reviewer name and their email address
...
def get_paper(paper_number):
    ...
    notes = client.get_notes(invitation='ICLR.cc/2017/workshop/-/submission', number = paper_number)
...
def get_data(invitation):
    ...
    anon_reviewers = requests.get(client.baseurl+'/groups?id=ICLR.cc/2017/workshop/paper.*/AnonReviewer.*', headers = headers)
    current_reviewers = requests.get(client.baseurl+'/groups?id=ICLR.cc/2017/workshop/paper.*/reviewers', headers = headers)
    notes = client.get_notes(invitation='ICLR.cc/2017/workshop/-/paper.*/' + invitation)
