...
var getRootGroups = function() {
  ...
  var allAreaChairGroupsP = Webfield.getAll('/groups', {
    regex: CONFERENCE_ID + '/Commitment.*/' + REVIEWER_NAME + '$',
    select: 'id'
  })
  .then(function(groups) {
    ...
    areaChairGroups.forEach(function(areaChairGroup) {
      var num = getNumberfromGroup(areaChairGroup.id, 'Commitment');
...
var getInvitationId = function(name, number) {
  return CONFERENCE_ID + '/Commitment' + number + '/-/' + name;
};

var getInvitationRegex = function(name, numberStr) {
  return '^' + CONFERENCE_ID + '/Commitment(' + numberStr + ')/-/' + name;
};
...
var getOfficialReviews = function(notes) {
  ...
  _.forEach(notes, function(n) {
    var anonId = getNumberfromGroup(n.signatures[0], 'Ethics_Reviewer_');
...
var getReviewerGroups = function(noteNumbers) {
  ...
  return Webfield.getAll('/groups', {
    regex: CONFERENCE_ID + '/Commitment.*/Ethics_Reviewer',
    select: 'id,members'
  })
  .then(function(groups) {
    var anonGroups = _.filter(groups, function(g) { return g.id.includes('/Ethics_Reviewer_'); });
    var reviewerGroups = _.filter(groups, function(g) { return g.id.endsWith('/Ethics_Reviewers'); });

    _.forEach(reviewerGroups, function(g) {
      var num = getNumberfromGroup(g.id, 'Commitment');
      if (num in noteMap) {
          g.members.forEach(function(member, index) {
            var anonGroup = anonGroups.find(function(g) { return g.id.startsWith(CONFERENCE_ID + '/Commitment' + num + '/Ethics_Reviewer_') && g.members[0] == member; });
            if (anonGroup) {
              var anonId = getNumberfromGroup(anonGroup.id, 'Ethics_Reviewer_')
