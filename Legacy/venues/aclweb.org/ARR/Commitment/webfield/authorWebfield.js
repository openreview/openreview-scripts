...
var BLIND_SUBMISSION_ID = 'aclweb.org/ACL/2022/Workshop/WASSA/-/Blind_Submission';
...
function load() {
  ...
  var commitmentNotesP = Webfield.getAll('/notes', { invitation: COMMITMENT_SUBMISSION_ID, 'content.authorids': user.profile.id });

  return $.when(notesP, commitmentNotesP, getAllInvitations());
}
...
function renderContent(authorNotes, commitmentNotes, invitations) {
  // Author Tasks tab
  var tasksOptions = {
  ...
  renderCommitmentStatusTable(commitmentNotes);
...
function renderCommitmentStatusTable(notes) {
    var $container = $('#your-commitments');
    var rows = notes.map(function(note) {
      var invitationPrefix = CONFERENCE_ID + '/Paper' + note.number + '/-/';
      var decision = _.find(note.details.directReplies, ['invitation', invitationPrefix + DECISION_NAME]);

      return buildTableRow(note, [], decision);
    });
    ...
    var rowsHtml = rows.map(function(row) {
      ...
      var numberHtml = '<strong class="note-number">' + row[0].number + '</strong>';
      var summaryHtml = Handlebars.templates.noteSummary(row[1]);
      var decisionHtml = '<h4>' + (row[3].recommendation || 'No Decision') + '</h4>';

      return [numberHtml, summaryHtml, decisionHtml];
    });

    var tableHtml = Handlebars.templates['components/table']({
      headings: ['#', 'Paper Summary', 'Decision'],
      rows: rowsHtml,
      extraClasses: 'console-table'
    });
    ...
    $('#your-submissions .console-table th').eq(0).css('width', '4%');
    $('#your-submissions .console-table th').eq(1).css('width', '48%');
    $('#your-submissions .console-table th').eq(2).css('width', '48%');
  }
