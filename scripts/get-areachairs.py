...
parser.add_argument('conference',help="the base path of the conference whose areachairs you would like to see. This should correspond to the prefix of the submission invitation. (E.g. if the submission invitation is ICLR.cc/2017/conference/-/submission, the prefix is ICLR.cc/2017/conference")
...
submissions = openreview.get_notes(invitation=args.conference+'/-/submission')
...
if args.paper_number!=None:
    ...
    try:
        note = notes[0]
        message = []
        areachairs = openreview.get_group(args.conference+'/paper'+str(note.number)+'/areachairs');
...
if args.user!=None:
    user = args.user
    groups = openreview.get_groups(member=user,regex=args.conference+'/paper[0-9]+/areachair[0-9]+')
    notes = openreview.get_notes(invitation=args.conference+'/-/submission')
    notesMap = {}
    for n in notes:
