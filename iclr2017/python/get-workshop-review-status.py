...
import csv
#####################################################################################
# This script produces a csv file with information on the review status of workshop papers
# that includes the paper number, type of note, reviewer name, score, recommendation, confidence
...
parser.add_argument('--ofile', help="output file name - default to ws-review-status.csv")
...
file_name = "ws-review-status.csv"
if args.ofile!=None:
...
area_chairs = requests.get(client.baseurl+'/groups?id=ICLR.cc/2017/conference/paper.*/areachair.*', headers = client.headers)
...
def check_profile(member):
    ...
    if '@' in member:
        ...
        #print member
        response = requests.get(client.baseurl+'/user/profile?email=' + member, headers = client.headers)
...
def get_paper_status_entry(note, paper_num, note_type):
    ...
    if reviewer not in paper_status[paper_num]:
        ...
        paper_status[paper_num][reviewer]['type'] = note_type
...
for note in allnotes:
    ...
    if paper_type == '/official/review':
        paper_entry = get_paper_status_entry(note, paper_num, paper_type)
        if paper_entry:
    ...
    #                    /official/comment, /public/review, /pre-review/question
        ...
        paper_entry = get_paper_status_entry(note, paper_num, '/')
        if paper_entry:
