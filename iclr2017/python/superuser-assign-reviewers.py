...
def assign_reviewer(reviewer,paper_number):
    ...
    if notes:
        ...
        reviewer_group,invited,tilde_found = get_reviewer_group(reviewer, paper_number, conflict_list)
...
def get_reviewer_group(reviewer, paper_number, conflict_list):
    ...
    try:
        reviewergroup = client.get_group(reviewer)
        tildegroups = [i for i in reviewergroup.members if re.compile('~.*').match(i)]
