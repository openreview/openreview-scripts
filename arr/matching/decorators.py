...
def require_confirmation(
    ...
    *,
    ...
    The decorator respects `dry_run` and `force` parameters if present in the function signature.
    - When `dry_run=True`, confirmation is skipped and the operation is not executed.
    - When `force=True`, confirmation is skipped but the operation is still executed.
    
    Example:
        @require_confirmation
        def delete_edges(self, invitation: str, label: Optional[str] = None, dry_run: bool = False, force: bool = False):
            # ... implementation
    ...
    def decorator(f: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            # Check for dry_run and force parameters
            sig = inspect.signature(f)
            bound_args = sig.bind(*args, **kwargs)
            bound_args.apply_defaults()
            
            dry_run = bound_args.arguments.get('dry_run', False)
            force = bound_args.arguments.get('force', False)
            if dry_run or force:
