...
class MatchingData:
    ...
    def _populate_registration_and_load(self, logger: logging.Logger) -> None:
        ...
        def resolve_profile_id(note: openreview.api.Note) -> Optional[str]:
            ...
            if isinstance(getattr(note, 'content', None), dict):
                ...
                if isinstance(raw, dict):
                    raw_profile_id = raw.get('value')
                elif isinstance(raw, str):
                    raw_profile_id = raw
            if raw_profile_id:
                return raw_profile_id
            if signature and signature in self.name_to_id:
                return self.name_to_id[signature]
            return None

        self.sac_reg = {}
        for note in sac_registration_notes:
            profile_id = resolve_profile_id(note)
            if profile_id:
                self.sac_reg[profile_id] = note

        self.ac_reg = {}
        for note in ac_registration_notes:
            profile_id = resolve_profile_id(note)
            if profile_id:
                self.ac_reg[profile_id] = note

        self.ac_loads = {}
        for note in ac_load_notes:
            profile_id = resolve_profile_id(note)
            if profile_id:
                self.ac_loads[profile_id] = note
...
class SACACMatching:
    ...
    def __init__(
        ...
        request_form_note = client_v1.get_note(request_form_id)
