...
def _as_string_list(value: Union[None, str, List[str]]) -> List[str]:
    if value is None:
        return []
    ...
    if isinstance(value, str):
        return [value]
    raise TypeError(f"Unsupported research area type: {type(value)}")
...
class MatcherInterface:
    ...
    @require_confirmation
    def post_matcher(self, config_note_id, dry_run: bool = False, force: bool = False):
...
class AssignmentsBuilder(object):
    def __init__(
        ...
        self.registration_loader = RegistrationDataLoader(self.client, self.venue)
    ...
    def sync_research_areas(
        ...
        profile_id_to_research_areas = self.registration_loader.get_research_areas(
            members.members,
            group_id=group_id
        )
    ...
    def post_research_areas(
        ...
        for submission in submissions:
            submission_research_areas = _as_string_list(
                submission.content.get('research_area', {}).get('value')
            )
            for research_area in submission_research_areas:
