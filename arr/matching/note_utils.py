...
class NoteUtils(object):
    ...
    def map_profile_id_to_note(
        ...
        profiles = openreview.tools.get_profiles(client, all_tilde_ids)
        name_to_id: Dict[str, str] = ProfileUtils.map_profile_names_to_profile_id(profiles)
        ...
        for note in notes:
            ...
            # Get canonical profile ID from the mapping
            if signature in name_to_id:
                profile_id = name_to_id[signature]
                id_to_note[profile_id] = note
        return id_to_note
