// Constants
var CONFERENCE_ID = 'thecvf.com/ECCV/2020/Conference';
var SHORT_PHRASE = 'ECCV 2020';
var SUBMISSION_ID = 'thecvf.com/ECCV/2020/Conference/-/Submission';
var BLIND_SUBMISSION_ID = 'thecvf.com/ECCV/2020/Conference/-/Blind_Submission';
var HEADER = {"title": "Area Chairs Console", "instructions": "<p class=\"dark\">This page provides information and status             updates for the ECCV 2020. It will be regularly updated as the conference             progresses, so please check back frequently.</p>", "schedule": "<h4>Coming Soon</h4>            <p>                <em><strong>Please check back later for updates.</strong></em>            </p>"};
...
var OFFICIAL_SECONDARY_META_REVIEW_NAME = 'Secondary_Meta_Review';
var SECONDARY_AREACHAIR_WILDCARD = CONFERENCE_ID + '/Paper.*/Secondary_Area_Chair$'
...
var renderStatusTable = function(profiles, notes, allInvitations, completedReviews, metaReviews, reviewerIds, acRankingByPaper, reviewerRankingByPaper, container) {
  ...
  var sortResults = function(newOption, switchOrder) {
    ...
    renderTable(_.orderBy(rows, sortOptions[newOption], order), container, showRankings);
  }
  ...
  if (rows.length) {
    renderTable(rows, container, showRankings);
  } else {
...
var renderTasks = function(invitations, edgeInvitations, tagInvitations) {
  ...
  var areachairInvitations = invitations;
...
var renderTableAndTasks = function(fetchedData) {
  ...
  renderStatusTable(
    ...
    _.filter(fetchedData.blindedNotes, function(note) { return fetchedData.acPapers.indexOf(note.number) > -1; }),
...
var buildTableRow = function(note, reviewerIds, completedReviews, metaReview, metaReviewInvitation, acPaperRanking, reviewerPaperRanking, typeMetaReviewer) {
  ...
  // Status cell
  var cell3InvitationId = getInvitationId('Meta_Review', note.number);
  if (typeMetaReviewer === 'secondary') {
    cell3InvitationId = getInvitationId('Secondary_Meta_Review', note.number);
  }
  var invitationUrlParams = {
    id: note.forum,
    noteId: note.id,
    invitationId: cell3InvitationId,
    referrer: referrerUrl
  };
  var cell3 = {
    noteId: note.id,
