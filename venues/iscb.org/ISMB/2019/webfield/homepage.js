...
function renderContent(notes, submittedNotes, assignedNotePairs, assignedNotes, userGroups, authorNotes, tagInvitations) {
  ...
  if (userGroups.length) {
    ...
    // AC console
    if (_.includes(userGroups, AREA_CHAIRS)) {
      $('#my-tasks .submissions-list').prepend([
        '<li class="note invitation-link">',
          '<a href="/group?id=' + AREA_CHAIRS + '">Area Chair Console</a>',
        '</li>'
      ].join(''));
    }

     // PC console
    if (_.includes(userGroups, PROGRAM_CHAIRS)) {
      $('#my-tasks .submissions-list').prepend([
        '<li class="note invitation-link">',
          '<a href="/group?id=' + PROGRAM_CHAIRS + '">Program Chairs Console</a>',
        '</li>'
      ].join(''));
    }
