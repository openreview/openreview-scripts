...
if __name__ == '__main__':
    ...
    with open('../webfield/reviewerWebfieldBiddingEnabled.js','r') as f:
        reviewers = client.get_group(akbc19.REVIEWERS_ID)
        reviewers.web = f.read()
        reviewers = client.post_group(reviewers)

    with open('../webfield/areachairWebfieldBiddingEnabled.js','r') as f:
        area_chairs = client.get_group(akbc19.AREA_CHAIRS_ID)
        area_chairs.web = f.read()
        area_chairs = client.post_group(area_chairs)

    akbc19.add_bid.invitees = [akbc19.REVIEWERS_ID, akbc19.AREA_CHAIRS_ID]
    client.post_invitation(akbc19.add_bid)
    ...
    client.post_invitation(akbc19.scores_inv)
    user_score_notes = {}
    for paper in openreview.tools.iterget_notes(client, invitation=akbc19.BLIND_SUBMISSION_ID):
        for user_id in valid_reviewer_ids + valid_ac_ids:
            if user_id not in user_score_notes:
                user_score_notes[user_id] = openreview.Note.from_json({
                    'invitation': akbc19.SCORES_INV_ID,
                    'readers': [user_id],
                    'writers': ['AKBC.ws/2019/Conference'],
                    'content': {
                        'user': user_id,
                        'scores': []
                    },
                    'signatures': ['AKBC.ws/2019/Conference'],
                    'forum': None,
                    'multiReply': False
                })
