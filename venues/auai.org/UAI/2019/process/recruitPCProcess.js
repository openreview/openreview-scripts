function() {
  ...
  if(hashKey == note.content.key) {
    if (note.content.response == 'Yes') {
      ...
      or3client.removeGroupMember('auai.org/UAI/2018/Program_Committee/Declined', note.content.email, token) //MODIFIED_BY_YCN: SPC->PC
      or3client.addGroupMember('auai.org/UAI/2018/Program_Committee', note.content.email, token) //MODIFIED_BY_YCN: SPC->PC
      .then(result => done())
      ...
      or3client.removeGroupMember('auai.org/UAI/2018/Program_Committee', note.content.email, token) //MODIFIED_BY_YCN: SPC->PC
      or3client.addGroupMember('auai.org/UAI/2018/Program_Committee/Declined', note.content.email, token) //MODIFIED_BY_YCN: SPC->PC
      .then(result => done())
      .catch(error => done(error));
    } else {
