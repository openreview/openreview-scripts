function() {
    ...
    .then(overwritingId => or3client.or3request(or3client.notesUrl + '?id=' + overwritingId, {}, 'GET', token))
    .then(result => {
      overwritingNote = result.notes[0];
      var reviewerMail = {
        "groups": ['ICLR.cc/2018/Conference/Paper' + number + '/Reviewers'],
        "subject": "Revision posted to a paper that you reviewed",
        "message": "A paper that you reviewed has been revised. \n\nTo view the paper, click here: " + baseUrl + "/forum?id=" + overwritingNote.forum + "\n\nYou can see the changes made at " + baseUrl + "/revisions?id=" + overwritingNote.forum
      };
