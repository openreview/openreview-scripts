...
submission_invitation = 'ICLR.cc/2021/Conference/-/Blind_Submission'

print('Get Submissions')
submissions = openreview.tools.iterget_notes(client, invitation=submission_invitation)

if not os.path.exists('iclr2021_pdfs'):
	os.makedirs('iclr2021_pdfs')
...
def get_pdf(submission):
	if 'pdf' in submission.content:
		...
		try:
			with open('iclr2021_pdfs/Paper{0}.pdf'.format(paper_number), 'wb') as f:
