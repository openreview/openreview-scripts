...
var COMMITMMENT_SUBMISSION_ID = 'aclweb.org/NAACL/2022/Conference/-/Commitment_Submission';
var SPECIAL_THEME_SUBMISSION_ID = 'aclweb.org/NAACL/2022/Conference/-/Special_Theme_Submission';
var SPECIAL_THEME_BLIND_SUBMISSION_ID = 'aclweb.org/NAACL/2022/Conference/-/Blind_Special_Theme_Submission';
var WITHDRAWN_SUBMISSION_ID = 'aclweb.org/NAACL/2022/Conference/-/Withdrawn_Special_Theme_Submission';
var DESK_REJECTED_SUBMISSION_ID = 'aclweb.org/NAACL/2022/Conference/-/Desk_Rejected_Special_Theme_Submission';
var HEADER = {"title": "Program Chairs Console", "instructions": "<p class=\"dark\">This page provides information and status             updates for the ACL-2022. It will be regularly updated as the conference             progresses, so please check back frequently.</p>"};
...
var COMMENT_NAME = 'Official_Comment';
var SCORES_NAME = 'Affinity_Score';
var AUTHORS_ID = 'aclweb.org/NAACL/2022/Conference/Authors';
...
var SENIOR_AREA_CHAIRS_ID = 'aclweb.org/NAACL/2022/Conference/Senior_Area_Chairs';
var PROGRAM_CHAIRS_ID = 'aclweb.org/NAACL/2022/Conference/Program_Chairs';
var REQUEST_FORM_ID = 'z-RGRVNOWNS';
...
var propertiesAllowed ={
    ...
    suggested_decision: ['suggestedDecision.content.suggested_decision'],
...
var main = function() {
  ...
  $.when(
    ...
    blindedNotes.forEach(function(submission) {
      ...
      var paperOfficialReviews = _.filter(submission.details.replies, ['invitation', 'aclweb.org/NAACL/2022/Conference/-/ARR_Official_Review']);
      var paperMetaReviews = _.find(submission.details.replies, ['invitation', 'aclweb.org/NAACL/2022/Conference/-/ARR_Meta_Review']);
      var paperDecisions = _.find(submission.details.replies, ['invitation', getInvitationId('Decision', submission.number)]) || { content: { decision: 'No Decision' } };
      var paperSuggestedDecisions = _.find(submission.details.replies, ['invitation', getInvitationId('Suggested_Decision', submission.number)]) || { content: { decision: 'No Decision' } };
      ...
      submission.details.suggestedDecision = paperSuggestedDecisions;
      ...
      var trackSACs = SACByTrack['aclweb.org/NAACL/2022/Conference/' + sacNameDictionary[submission.content.track] + '/Senior_Area_Chairs'].members;
    ...
    specialThemeNotes.forEach(function(submission) {
      selectedNotesById[submission.id] = false;
      var tags = submission.details.tags;
      tags.forEach(function(t) {
        if (t.invitation === AREA_CHAIRS_ID + '/-/Paper_Ranking') {
    ...
    conferenceStatusData = {
      ...
      specialThemeNotes: specialThemeNotes,
    ...
    var conferenceStats = {
      ...
      commitmentNotesCount: commitmentNotes.length,
      specialThemeNotesCount: specialThemeNotes.length,
...
var getTrackGroups = function() {
  return Webfield.get('/groups', { regex: 'aclweb.org/NAACL/2022/Conference/.*/Senior_Area_Chairs', select: 'id,members' })
  .then(function(result) {
...
var getSpecialThemeNotes = function() {
  return Webfield.getAll('/notes', {
    invitation: SPECIAL_THEME_BLIND_SUBMISSION_ID,
    details: 'invitation,tags,original,replyCount,replies',
    select: 'id,number,forum,content,details'
  })
};
...
var getEthicsReviewInvitations = function() {
  return Webfield.getAll('/invitations', {
    regex: 'aclweb.org/NAACL/2022/Conference/Paper.*/-/Ethics_Review',
    select: 'id'
  })
};

var getCommitmentNotes = function() {
  return Webfield.getAll('/notes', {
    invitation: COMMITMMENT_SUBMISSION_ID,
    details: 'invitation',
    sort: 'number:asc'
  });
};
...
var getGroups = function() {
  return Webfield.getAll('/groups', {
    id: CONFERENCE_ID + '/(Paper|Commitment).*',
    select: 'id,members'
    ...
    for (var groupIdx = 0; groupIdx < groups.length; groupIdx++) {
      ...
      if (group.id.endsWith('/Reviewers')) {
        ...
        var number = getNumberfromGroup(group.id, 'Commitment');
...
var getSpecialThemeOfficialReviews = function(notes) {
  var ratingExp = /^(\d+): .*/;
...
var renderHeader = function() {
  ...
  var tabs = [
    ...
    {
      heading: 'Special Theme Status',
      id: 'special-theme-status',
      content: loadingMessage,
      extraClasses: 'horizontal-scroll'
    }
  ];
...
var displayStatsAndConfiguration = function(conferenceStats) {
  ...
  html += renderStatContainer(
    ...
    '<h3>' + conferenceStats.commitmentNotesCount + '</h3>'
  );
  html += renderStatContainer(
    'Active Submissions:',
  ...
  html += renderStatContainer(
    ...
    '<h3>' + conferenceStats.specialThemeNotesCount + '</h3>'
  );
  html += renderStatContainer(
  ...
  pushToDatedArrays(invitationMap, COMMITMMENT_SUBMISSION_ID, 'Commitment Submissions');
  ...
  var SAC_groups = ["Ethics_NLP","LCMP","Machine_Learning_NLP","Phonology_Morphology_Word_Segmentation","Resources_Evaluation","Semantics_Lexical","Semantics_STO","Syntax_TCP","Information_Extraction","CSSCA","Info_Retrieval_Text_Mining","IAM_for_NLP","Machine_Translation_Multilinguality","NLP_Applications","Question_Answering","Dialogue_and_Interactive_Systems","Discourse_and_Pragmatics","Generation","LGVRB","SASAAM","Speech_and_Multimodality","Summarization","Special_Theme"];
  SAC_groups.forEach(function(SAC_group){
    html += '<li><a href="/group/edit?id=' + CONFERENCE_ID + '/'+SAC_group + '/Senior_Area_Chairs">' + SAC_group + '</a></li>';
  });
  if (SENIOR_AREA_CHAIRS_ID) {
...
var displayCommitmentStatusTable = function() {

  var container = '#commitment-status';
  var notes = conferenceStatusData.commitmentNotes;
  ...
  var renderTable = function(container, data) {
    ...
    var rowData = _.map(data, function(d) {
      ...
      var rows = [numberHtml, summaryHtml];

      return rows;
    });

    var headings = ['#', 'Paper Summary'];
    ...
    $container.off('click', 'ul.pagination > li > a').on('click', 'ul.pagination > li > a', function(e) {
      ...
      postRenderTable1(data, newPageNum);
      return false;
    });

  };

  var postRenderTable1 = function(data, pageNum) {
    $(container + ' .console-table th').eq(0).css('width', '3%');
    $(container + ' .console-table th').eq(1).css('width', '97%');
  ...
  if (rowData.length) {
    ...
    '</div>' +
    '<div class="btn-group"><button class="btn btn-export-commitments">Export</button></div>');
    renderTable(container, rowData);
  } else {
...
var displayPaperStatusTable = function() {
  ...
  sortOptions['Suggested_Decision'] = function (row) { return row.suggestedDecision.content.suggested_decision || 'No Decision' }
  ...
  var renderTable = function(container, data) {
    ...
    var rowData = _.map(data, function(d) {
      ...
      var assignedSACHtml = '<div>' +
      '<h4><a target="_blank" href="/group/info?id=aclweb.org/NAACL/2022/Conference/' + sacNameDictionary[d.note.content.track] + '/Senior_Area_Chairs">Assigned SACs</a></h4>' +
      '<lu>' +
          d.note.details.assignedSACs.map(function(a) { return '<li><a target="_blank" href="/profile?id=' + a + '">' + view.prettyId(a) + '</a></li>'; }).join('\n') +
      '</lu>' +
      '</br>' +
      '<h4><a target="_blank" href="/group/info?id=aclweb.org/NAACL/2022/Conference/Commitment' + d.note.number + '/Conflicts">Conflicted SACs</a></h4>' +
      '<lu>' +
      ...
      '</lu>' +
      '</div>';
      var suggestedDecisionHtml = '<h4>' + (d.suggestedDecision.content.suggested_decision || 'No Decision') + '</h4>';
      ...
      rows.push(assignedSACHtml);
      rows.push(suggestedDecisionHtml);
    ...
    var pageNum = $container.data('lastPageNum') || 1;
    renderPaginatedTable($container, tableData, pageNum);
    postRenderTable2(container, data, pageNum);

    $container.off('click', 'ul.pagination > li > a').on('click', 'ul.pagination > li > a', function(e) {
      paginationOnClick($(this).parent(), $container, tableData);

      var newPageNum = parseInt($(this).parent().data('pageNumber'), 10);
      postRenderTable2(container, data, newPageNum);
      return false;
    });
  ...
  var postRenderTable2 = function(container, data, pageNum) {
    $(container + ' .console-table th').eq(0).css('width', '3%');
    $(container + ' .console-table th').eq(1).css('width', '5%');
    if (AREA_CHAIRS_ID) {
      $(container + ' .console-table th').eq(2).css('width', '21%');
      $(container + ' .console-table th').eq(3).css('width', '21%');
      $(container + ' .console-table th').eq(4).css('width', '14%');
      $(container + ' .console-table th').eq(5).css('width', '15%');
      $(container + ' .console-table th').eq(6).css('width', '10%');
      $(container + ' .console-table th').eq(7).css('width', '10%');
    } else {
      $(container + ' .console-table th').eq(2).css('width', '64%');
      $(container + ' .console-table th').eq(3).css('width', '20%');
    }
...
var displaySpecialThemeStatusTable = function() {

  var container = '#special-theme-status';
  var pcAssignmentTagInvitations = conferenceStatusData.pcAssignmentTagInvitations;

  var rowData = _.map(conferenceStatusData.specialThemeNotes, function(note) {
    return buildSpecialThemeTableRow(note);
  });
  var filteredRows = null;
  var toNumber = function(value) {
    return value === 'N/A' ? 0 : value;
  ...
  var searchResults = function(searchText, isQueryMode) {
    ...
    renderTable(container, filteredRows);
    $(container + " .btn-export-special-theme").text("Export " + filteredRows.length + " records");
  };
  ...
  if (rowData.length) {
    ...
    $(container).find('form.search-form .pull-left').html('<div class="btn-group message-papers-container" role="group">' +
      ...
      '</ul>' +
    '</div>' +
    '<div class="btn-group"><button class="btn btn-export-special-theme" type="button">Export ' + rowData.length + ' records</button></div>');
    renderTable(container, rowData);
  } else {
    $(container).empty().append('<p class="empty-message">No papers have been submitted. ' +
      'Check back later or contact info@openreview.net if you believe this to be an error.</p>');
...
var buildSpecialThemeTableRow = function(note) {
...
var buildPaperTableRow = function(note) {
  ...
  var metaReview = note.details.metaReview;
  var decision = note.details.decision;
  var suggestedDecision = note.details.suggestedDecision;
  var areachairProfile = [];
  var seniorAreaChairProfile = [];
  ...
  var areachairProgressData = {
    ...
    referrer: paperTableReferrerUrl,
    // actions: [
    //   { name: 'Assigned SACs', url: '/group/info?id=aclweb.org/NAACL/2022/Conference/' + sacNameDictionary[note.content.track] + '/Senior_Area_Chairs' },
    //   { name: 'Conflicts detected', url: '/group/info?id=aclweb.org/NAACL/2022/Conference/Paper' + note.number + '/Conflicts' }
    // ]
  };
  if (seniorAreaChairProfile.length) {
    var seniorAreaChair = {
...
$('#group-container').on('shown.bs.tab', 'ul.nav-tabs li a', function(e) {
  ...
  if (containerId === '#commitment-status') {
    displayCommitmentStatusTable();
  } else if (containerId === '#paper-status') {
    displayPaperStatusTable();
  } else if (containerId === '#special-theme-status') {
    displaySpecialThemeStatusTable();
...
var buildSpecialThemeCSV = function(){
  ...
  var notes = conferenceStatusData.specialThemeNotes
...
var buildCSV = function(){
  ...
  _.forEach(notes, function(noteObj) {
    ...
    var suggestedDecision = paperTableRow.suggestedDecision;
    ...
    '"' + (paperTableRow.note.content.comment || '').replace(/"/g, '""') + '"',
    '"' + paperTableRow.note.content.commitment_note + '"',
    '"' + paperTableRow.note.content.naacl_preprint + '"',
    '"' + paperTableRow.note.content.preprint + '"',
    '"' + paperTableRow.note.content.existing_preprints + '"',
    '"' + paperTableRow.note.content.previous_URL + '"',
    '"' + paperTableRow.note.content.country_of_affiliation_of_corresponding_author + '"',
    ...
    '"' + metareviewScore + '"',
    '"' + actionEditor + '"',
    '"' + ((suggestedDecision && suggestedDecision.content.suggested_decision) || '' ) + '"',
    '"' + ((suggestedDecision && suggestedDecision.content.justification) || '').replace(/"/g, '""')  + '"',
    '"' + ((suggestedDecision && suggestedDecision.content.ranking) || '' ) + '"',
    '"' + ((suggestedDecision && suggestedDecision.content.best_paper) || '' ) + '"',
    '"' + hasEthicsReview + '"',
...
var buildCommitmentsCSV = function(){
  var notes = conferenceStatusData.commitmentNotes;
  ...
  _.forEach(notes, function(note) {
    var title = note.content.title.replace(/"/g, '""');
    rowData.push([note.number,
    '"https://openreview.net/forum?id=' + note.id + '"',
    '"' + title + '"',
    '"' + note.content.authors.join('|') + '"',
    '"' + note.content.paper_link + '"',
    '"' + note.content.paper_type + '"',
    '"' + note.content.track + '"',
...
$('#group-container').on('click', 'button.btn.btn-export-special-theme', function(e) {
  var blob = new Blob(buildSpecialThemeCSV(), {type: 'text/csv'});
  var fileName = SHORT_PHRASE.replace(/\s/g, '_') + '_special_theme_status.csv'
  saveAs(blob, fileName);
});


$('#group-container').on('click', 'button.btn.btn-export-commitments', function(e) {
  var blob = new Blob(buildCommitmentsCSV(), {type: 'text/csv'});
  saveAs(blob, SHORT_PHRASE.replace(/\s/g, '_') + '_commitments_status.csv',);
});
