def process(client, note, invitation):
   ...
   # Mail to the authors 
    email_subject = '''{CONFERENCE_SHORT_NAME}: Decision posted to your Paper #{paper_number} titled "{paper_title}" by program chairs'''.format(
        CONFERENCE_SHORT_NAME=CONFERENCE_SHORT_NAME,
        paper_number=forum_note.number,
        paper_title=forum_note.content['title']
        
    )
    email_body = f'A decision was posted to your Paper #{forum_note.number} titled "{forum_note.content["title"]}".\n\n Decision: {note.content["decision"]}.'
