...
// CONFERENCE + '/-/Paper' + numb er + '/Official_Review'

// Constants
var HEADER_TEXT = 'IEEE 2018 ITSC Program Chairs Console';

var CONFERENCE = 'IEEE.org/2018/ITSC';
...
var ANONREVIEWER_REGEX = /^IEEE\.org\/2018\/ITSC\/Paper(\d+)\/AnonReviewer(\d+)/;
...
var displayPaperStatusTable = function(profiles, notes, completedReviews, reviewerIds, container, options) {
  ...
  var rowData = _.map(notes, function(note) {
    ...
    return buildPaperTableRow(note, revIds, completedReviews[note.number]);
  });
...
var displayPCStatusTable = function(profiles, notes, completedReviews, reviewerByNote, reviewerById, container, options) {
  var rowData = [];
  var index = 1;
...
// Helper functions
var buildPaperTableRow = function(note, reviewerIds, completedReviews) {
...
controller.addHandler('programChairsConsole', {
  token: function(token) {
    ...
    .then(function(blindedNotes, officialReviews, reviewerGroups, loaded) {
      ...
      return getUserProfiles(uniqueReviewerIds)
      .then(function(profiles) {
        displayPaperStatusTable(profiles, blindedNotes, officialReviews, reviewerGroups.byNotes,'#paper-status');
        displayPCStatusTable(profiles, blindedNotes, officialReviews, reviewerGroups.byNotes, reviewerGroups.byReviewers, '#reviewer-status');
      })
    })
    .fail(function(error) {
      displayError();
...
$('#group-container').on('click', 'a.send-reminder-link', function(e) {
  ...
  var postData = {
    subject: 'IEEE 2018 ITSC Reminder',
    message: 'This is a reminder to please submit your official reviews for IEEE 2018 ITSC. ' +
